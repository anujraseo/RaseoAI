      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button style={tabStyle(activeTab === 'audits')} onClick={() => setActiveTab('audits')}>
            📊 Audits {data ? `(${data.recentAudits.length})` : ''}
          </button>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 7, color: loading ? 'rgba(255,255,255,0.3)' : 'rgba(255,255,255,0.6)',
              padding: '7px 16px', fontSize: 12, cursor: loading ? 'not-allowed' : 'pointer',
              fontFamily: 'var(--font-dm)',
            }}
          >
            {loading ? '↻ Loading…' : '↻ Refresh'}
          </button>
          <a href="/" style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', textDecoration: 'none' }}>
            ← Back to Tool
          </a>
        </div>
      </nav>
        ...
        {loading && !data ? (
          ...
          <>
            <div style={{ marginBottom: 24 }}>
              <h1 style={{ fontFamily: 'var(--font-syne)', fontSize: 24, fontWeight: 800, color: '#fff', marginBottom: 4 }}>
                Dashboard
              </h1>
              <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.35)' }}>
                ai-seoaudit.com — All-time analytics
              </p>
            </div>
